...
(function(){
  ...
  // Dedicated Explore Viewer Zoom & Pan controller
  ...
  function getActiveViewerImg() {
    return currentViewerSlot === 'A' ? exploreViewerImgA : exploreViewerImgB;
  }

  function applyViewerTransform(animated) {
    const img = getActiveViewerImg();
    if (!img) return;
    if (animated) {
      img.style.transition = 'transform 0.28s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.35s ease';
    } else {
    ...
    if (viewerZoomScale <= 1.02) {
      ...
      img.style.transform = `translate(calc(-50% + ${viewerPanX}px), calc(-50% + ${viewerPanY}px)) scale(${viewerZoomScale})`;
    }
  }

  function resetViewerZoom(animated = true) {
    viewerZoomScale = 1;
    viewerPanX = 0;
    viewerPanY = 0;
    ...
    if (exploreViewerImgA) {
      exploreViewerImgA.style.transform = '';
      exploreViewerImgA.style.transition = '';
    }
    if (exploreViewerImgB) {
      exploreViewerImgB.style.transform = '';
      exploreViewerImgB.style.transition = '';
    }
  }
  ...
  function imageVariantUrl(imageUrl, variant) {
    ...
    if (cleanParts.length >= 2) {
      ...
      return `images/${section}/${variant}/${filename}`;
    }
    return `images/${variant}/${clean}`;
  }
  ...
  function updateExploreViewerPhoto(imgUrl) {
    ...
    const applyImage = (loadedImg) => {
      ...
      if (typeof resetViewerZoom === 'function') resetViewerZoom(false);
    };
  ...
  /* =========================================================
     EXPLORATION MODE (FRAMELESS BOTTOM CARDS)
     ========================================================= */
  function enterExploreMode(categoryKey, targetPhotoUrl) {
    ...
    stage.classList.add('explore-mode', 'explore-cards-intro');
  ...
  function exitExploreMode() {
    ...
    if (exploreCardsIntroTimer) clearTimeout(exploreCardsIntroTimer);
    exploreCardsIntroTimer = null;
    stage.classList.remove('explore-mode', 'explore-cards-intro');
    if (exploreViewerImgA) {
  ...
  if (exploreViewerContainer) {
    exploreViewerContainer.addEventListener('touchstart', (e) => {
      ...
      if (e.touches.length > 1) {
        ...
        pinchStartScale = viewerZoomScale;
        touchStartX = null;
        touchStartY = null;
      } else if (e.touches.length === 1 && viewerZoomScale > 1.05) {
        // Pan with single finger while zoomed in
        isPanningViewer = true;
        panStartX = e.touches[0].clientX - viewerPanX;
        panStartY = e.touches[0].clientY - viewerPanY;
        touchStartX = null;
        touchStartY = null;
      }
    }, { passive: true });
    ...
    exploreViewerContainer.addEventListener('touchmove', (e) => {
      if (!inExploreMode) return;
      if (e.touches.length > 1 && isPinchingViewer) {
        isPinchGesture = true;
        lastPinchEndTime = performance.now();
        touchStartX = null;
        touchStartY = null;
        ...
        if (pinchStartDist > 0) {
          ...
          viewerZoomScale = Math.min(Math.max(0.85, pinchStartScale * factor), 3.8);
          applyViewerTransform(false);
        }
      } else if (e.touches.length === 1 && isPanningViewer && viewerZoomScale > 1.05) {
        touchStartX = null;
        touchStartY = null;
        const curX = e.touches[0].clientX;
        const curY = e.touches[0].clientY;
        const maxPanX = (window.innerWidth * (viewerZoomScale - 1)) / 2 + 40;
        const maxPanY = (window.innerHeight * (viewerZoomScale - 1)) / 2 + 40;
        viewerPanX = Math.max(-maxPanX, Math.min(maxPanX, curX - panStartX));
        viewerPanY = Math.max(-maxPanY, Math.min(maxPanY, curY - panStartY));
        applyViewerTransform(false);
      }
    }, { passive: true });
    ...
    exploreViewerContainer.addEventListener('touchcancel', () => {
      isPinchingViewer = false;
      isPanningViewer = false;
      touchStartX = null;
      touchStartY = null;
    ...
    // Double tap on image toggles zoom (1x <-> 2.2x), click edges on desktop only
    exploreViewerContainer.addEventListener('click', (e) => {
      if (!inExploreMode) return;
      if (e.target.closest('button, a')) return;

      const now = performance.now();
      const isQuickDoubleTap = (now - lastViewerTapTime < 320);
      ...
      if (isMobile) return; // On mobile, photo navigation is strictly via swipe

      if (viewerZoomScale > 1.05 || (performance.now() - lastPinchEndTime < 500)) return;
  ...
  stage.addEventListener('touchstart', (event) => {
    ...
    if (inExploreMode && viewerZoomScale > 1.05) {
      touchStartX = null;
      touchStartY = null;
      return;
    }
    if (event.target.closest('input, textarea, select, button, a') || 
        ...
        event.target.closest('.explore-top-bar') ||
  ...
  stage.addEventListener('touchend', (event) => {
    if (isPinchGesture || event.touches.length > 0 || (performance.now() - lastPinchEndTime < 650)) {
      touchStartX = null;
      touchStartY = null;
    ...
    if (inExploreMode) {
      ...
      } else if (absY > absX && deltaY < -75 && (performance.now() - lastPinchEndTime > 800)) {
        exitExploreMode();
      }
      return;
    }
